...
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("SELECT usuarios.*, rol.nombre AS rol FROM usuarios LEFT JOIN rol ON usuarios.rol_id = rol.id WHERE correo = ?");
...
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("UPDATE usuarios SET correo = ?, nombre = ?, telefono = ?, rol_id = ? WHERE id = ?");
	return $sentencia->execute([$usuario->correo, $usuario->nombre, $usuario->telefono, $usuario->rol_id, $usuario->id]);
}

function obtenerUsuarioPorId($idUsuario)
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("SELECT usuarios.id, usuarios.correo, usuarios.nombre, usuarios.telefono, usuarios.rol_id, rol.nombre AS rol
    FROM usuarios
    LEFT JOIN rol ON usuarios.rol_id = rol.id
    WHERE usuarios.id = ?");
	$sentencia->execute([$idUsuario]);
...
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->query("SELECT usuarios.id, usuarios.correo, usuarios.nombre, usuarios.telefono, usuarios.rol_id, rol.nombre AS rol
    FROM usuarios
    LEFT JOIN rol ON usuarios.rol_id = rol.id");
	return $sentencia->fetchAll();
}
...
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->query("SELECT * FROM rol");
	return $sentencia->fetchAll();
}
...
{
	$bd = conectarBaseDatos();
	$sentencia = $bd->prepare("INSERT INTO usuarios (correo, nombre, telefono, password, rol_id) VALUES(?,?,?,?,?)");
	return $sentencia->execute([$usuario->correo, $usuario->nombre, $usuario->telefono, $usuario->password, $usuario->rol_id]);
}
